...
for ($k=0;$k<$nx;$k++)
{
	...
	//$result= mysql_query("select count(*) from QueueKab where predvar<'2' and kab between " .$m1. " and ". $m2 ." group by kab order by 1 desc limit 1",$db)  or die("Query failed : " . mysql_error()); 
	$result= mysql_query("select count(*) from QueueKab where kab between " .$m1. " and ". $m2 ." group by kab order by 1 desc limit 1",$db)  or die("Query failed : " . mysql_error()); 
	...
	for ($j=0;$j<$MaxOrder;$j++)
		...
		for ($i=0;$i<(((($k+1)*$maxcol)>count($KS))?$dcol:$maxcol);$i++)
			...
			$result= mysql_query("select * from QueueKab where kab=". $KS[$i+$k*$maxcol]->PhN ." order by time_s limit ". $j .",1")  or die("Query failed : " . mysql_error()); 
			$NR=mysql_num_rows($result);
			if ($NR>0)
			{
		$myrow=mysql_fetch_row($result);
		$res_per=mysql_query("select * from pereriv where type_per=".$myrow[2]); 
                            $num_per=mysql_fetch_row($res_per);
                            if ($num_per){
                            echo " <p style='margin:0 auto; pading:0; color:".$myrow[10]."'>".$myrow[1].
                            "<br>(". date("H:i",strtotime($myrow[3])) ."-". date("H:i",strtotime($myrow[4])) .")</p>" ;
                            }
                            else{
                                echo " <a href=remove_t.php?id=".$myrow[8] ."&old_kab=". $KS[$i+$k*$maxcol]->PhN ." target=_top>"."<font color=".$myrow[10].">". $myrow[1]." </font>"  .
