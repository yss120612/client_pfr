...
$current_id=0;
 ...
 $link_admin="<li><a href=\"/admcs/adm_new/index.php\" >Администрирование КС</a></li>";
 ...
 while ($massivK = mysqli_fetch_row($queryK))
	{
		if ($ii==0)
		{
			$link_spec=$link_spec."<li><a href=\"#\">Кабинки c ". ($jj*$group_count+1) ." по ". (($jj+1)*$group_count) ."</a>\n";
			$link_spec=$link_spec."<ul>\n";
		}
		//не только свою
		if ($nomer_kab==$massivK[0])
		{
		$link_spec=$link_spec."<li><a href=\"kab/kab_main.php?nomer_kab={$massivK[0]}&rajon={$raion}&sub_rajon={$sub_raion}\">{$massivK[1]}</a></li>\n";
		}
