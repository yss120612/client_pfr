...
$sql= "SELECT a.FIO,a.kab,CONCAT(b.family,' ',SUBSTRING(b.name,1,1),'.',SUBSTRING(b.father,1,1),'.'),b.otrab,a.office,a.active,b.time_nach_p,time(p.b_start),a.priem,a.id FROM specialict a 
...
echo "<TR>\n";
echo "<TH width='{$ColWidth[0]}' class='text-center'>  Кабинка (прием) </TH>\n";
...
if(is_array($R[$i])){
	...
	if ($R[$i][6]<3)
	{
	    echo "<td class=\"text-center\"><input type=\"checkbox\" " . ($R[$i][8]=='1'?"checked=\"checked\"":"") ." id=\"".$R[$i][9]."\" onchange=\"kab_active('".$R[$i][9]."')\">". $R[$i][1] ."</input></td>\n";
	}
	else
	{
...
echo "<TH class='text-center'>  Посетитель </TH>\n";
