...
$result0 = mysqli_query($dbI,"SELECT FIO,id FROM specialict Where reg={$raion} AND office={$sub_raion} order by kab"); 
$MaxKab=mysqli_num_rows($result0);
while($tab_sql=mysqli_fetch_array($result0))
{
$kabs[]=$tab_sql[1];
echo "<th class='text-center'>{$tab_sql[0]}<BR>({$tab_sql[0]})</th>";
}
...
//$result = mysqli_query($dbI,"Select full_name,id+10000 from group_types_obr where actual='1' order by full_name");
$result = mysqli_query($dbI,"Select full_name,id+10000 from group_types_obr go where (select count(*) from types_obr where actual='1' and group_id=go.id)>0 order by id");
