...
if (isset($_REQUEST['switch']) && isset($_REQUEST['idspec']))
{
$sql="update specialict set priem='".$_REQUEST['switch']."' where id=".$_REQUEST['idspec'];
 mysqli_query($dbI,$sql) or die("Query failed U: " .$sql . mysqli_error($dbI));
 header ("location:/main/main.php",true);
}
...
echo "<form id=\"queForm\" action=\"main.php\" method=\"post\" role=\"form\">\n";
echo "<div id=\"que\"></div>\n";
echo "<input type=\"hidden\" id=\"switch\" name=\"switch\" value=\"0\">\n";
echo "<input type=\"hidden\" id=\"idspec\" name=\"idspec\" value=\"0\">\n";
echo "</form>\n";
/*
0-Не определено
2-Диспетчер
...
$result = mysqli_query($dbI, "select full_name,id from group_types_obr go where (select count(*) from types_obr where actual='1' and group_id=go.id)>0 order by id") or die("Query failed : " . mysqli_error());	
...
{ 
	...
	// {
		...
		// $tag_opened=true;
	// }
	echo "<option class=\"warning\" value=\"".($tab_sql[1]+10000)."\" ".($og==0?" selected ":"").">".$tab_sql[0]."</option>\n";
	$og+=1;
}
//if ($tag_opened){echo "</optgroup>\n";}
echo "</select>\n</td>\n";
...
function kab_active(v)
{
	
	$('#idspec').val(v);
	if ($('input[type=checkbox]#'+v).prop('checked'))
	{
		$('#switch').val('1');
	}
	else
